...
-- ===================================================== war bridge check ==
-- Console-callable proof that bridge/war_hook.lua is loaded AND is actually
-- choosing spawn templates from the war state, not falling through to stock
-- GCW behaviour.
--
-- WHY this exists: war_hook.lua only printf()s on its ERROR paths (malformed
-- region_map / war_state), so a silent log is ambiguous -- it means either
-- "loaded fine" or "never ran at all". This gives a positive signal.
--
-- WHY it is needed even though we can read war_state.lua from the host: the
-- question is not what the war state SAYS, it is whether the running server's
-- Lua VM has the hook installed over CityScreenPlay and resolves templates
-- through it. Only in-VM execution answers that.
--
-- Run from the server console (see CLAUDE.md "How to verify a change"):
--   screen -S swgemu-server -X stuff 'test warBridgeCheck\n'
--   grep WARBRIDGECHECK ~/workspace/Core3/MMOCoreORB/bin/screenlog.0
